...
function Notices() {
    ...
    function newFunction(){
        axios.get("http://localhost:5000/api/notice/userId="+authContext.user._id,{ headers: { 'Content-Type': 'application/json', 'X-Auth-Token': `${localStorage.getItem('token')}` } }).then((res)=>{
            setNotices(res.data);
        })
    }
    const NoticeData = (e) =>{
    ...
    const DeleteNotice = (e) =>{
        ...
        axios.post("http://localhost:5000/api/notice/remove_notice",body).then((res)=>{
