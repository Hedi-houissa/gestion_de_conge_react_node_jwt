...
function MyAccount() {
    ...
    const ModifierProfile = (e) =>{
        ...
        axios.patch("http://localhost:5000/api/user/update",params,{headers: {'Content-Type':'multipart/form-data','X-Auth-Token': `${localStorage.getItem('token')}`}}).then((res)=>{
            UpdatedToast();
        }).catch((error) => console.log(error))
    }
    return ( 
        <Box marginTop="-30px" display="flex" flexDirection="column" width="100%" bg="white" mx="25%" h="fit-content" borderRadius="5px">
                <Avatar src={"http://localhost:5000"+authContext.user.picture} size={'2xl'} pos={'relative'} left={'70%'} top={5}/>
