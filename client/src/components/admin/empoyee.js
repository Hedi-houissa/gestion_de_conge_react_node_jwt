...
function Employees() {
    ...
    const AddEmployee = (e) =>{
        ...
        e.preventDefault();
        axios.post('http://localhost:5000/api/user/add_user',body,{headers}).then((res)=>{
            AddedToast();
        },(error) =>{
    ...
    const DeleteEmployee = (e) => {
        ...
        e.preventDefault();
        axios.post('http://localhost:5000/api/user/delete_user',body,{headers}).then((res)=>{
            DeleteToast();
        },(error)=>{
            console.log(error.response);
        })
     ...
     function newFunction() {
                 axios.get('http://localhost:5000/api/user/all', { headers: { 'Content-Type': 'application/json', 'X-Auth-Token': `${localStorage.getItem('token')}` } })
                    .then((res) => {
                        setRows(res.data);
                    });
            }
