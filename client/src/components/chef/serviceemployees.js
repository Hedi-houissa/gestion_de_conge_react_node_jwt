...
function ServiceEmployee() {
    ...
    const DeleteEmployee = (e) =>{
        ...
        axios.post("http://localhost:5000/api/service/delete_from_service",body).then((res)=>{
            console.log(res.data);
        })
    }
    
    function newFunction (){
        axios.post("http://localhost:5000/api/service/commun_employees",body).then((res)=>{
        ...
        <Table>
            ...
            {Service_Employees.Employees.map(employee =>
                ...
                <Tr key={employee.id} id={employee.id}>
                    <Td><Avatar  name={employee.firstName +' '+ employee.lastName} src={'http://localhost:5000'+employee.picture}></Avatar></Td>
